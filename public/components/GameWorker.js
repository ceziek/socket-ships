...
importScripts('components/Ship.js');
...
class GameWorker {
    ...
    initPlayer() {
        ...
        const newPlayer = new Ship(id, initialPlayerState, playerBounds);
        const data = {
            id: newPlayer.id,
        ...
        for (let key in this.entities) {
            ...
            if (!state.hasOwnProperty(entityId)) {
                ...
                if (entityId === playerId) {
                    this.controlPlayer(entityId);
                    this.saveEntityState(entityId)
                }
            }
        }
    }
        ...
        for (let key in state) {
            if (state.hasOwnProperty(key)) {
                if (!this.entities.hasOwnProperty(key)) {
                    this.entities[key] = new Ship(key, state[key], playerBounds);
                } else {
